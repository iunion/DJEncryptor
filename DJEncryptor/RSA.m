...
NSString *const kDJRSAErrorDomain = @"kDJRSAErrorDomain";

NSString *const kDJRSAPubKeyTag = @"DJRSA_PublicKey";
NSString *const kDJRSAPrivKeyTag = @"DJRSA_PrivateKey";

@implementation NSError(RSA)
...
+ (NSError *)errorWithRSADescription:(NSString *)description reason:(NSString *)reason status:(OSStatus)status
{
    ...
    NSError *result = [NSError errorWithDomain:kDJRSAErrorDomain code:status userInfo:userInfo];
...
+ (NSData *)dataTrimFromPublicPemKey:(NSString *)key
{
    ...
    NSData *data = [NSString base64DecodeString:key];
...
+ (NSData *)encryptWithString:(NSString *)string publicPemKey:(NSString *)pubKey error:(NSError **)error
{
    ...
    return [RSA encryptWithData:utfData publicPemKey:pubKey error:error];
}
...
+ (NSData *)encryptWithData:(NSData *)data publicPemKey:(NSString *)pubKey error:(NSError **)error
{
    ...
    NSData *publicKeyData = [RSA dataTrimFromPublicPemKey:pubKey];
    ...
    NSData *result = [RSA encryptData:data withKeyRef:publicKey error:error];
...
{
    NSData *encryptData = [RSA encryptWithString:string publicPemKey:pubKey error:error];
    if (encryptData)
    {
        return [NSString base64EncodeData:encryptData];
    }
...
{
    NSData *encryptData = [RSA encryptWithData:data publicPemKey:pubKey error:error];
    if (encryptData)
    {
        return [NSString base64EncodeData:encryptData];
    }
...
+ (NSData *)encryptWithString:(NSString *)string publicDer:(NSData *)derFileData error:(NSError **)error
{
    ...
    return [RSA encryptWithData:utfData publicDer:derFileData error:error];
}
...
{
    NSData *encryptData = [RSA encryptWithString:string publicDer:derFileData error:error];
    if (encryptData)
    {
        return [NSString base64EncodeData:encryptData];
    }
...
{
    NSData *encryptData = [RSA encryptWithData:data publicDer:derFileData error:error];
    if (encryptData)
    {
        return [NSString base64EncodeData:encryptData];
    }
...
+ (NSData *)decryptWithString:(NSString *)string publicPemKey:(NSString *)pubKey error:(NSError **)error
{
    ...
    NSData *utfData = [NSString base64DecodeString:string];
    
    return [RSA decryptWithData:utfData publicPemKey:pubKey error:error];
}
...
+ (NSData *)decryptWithData:(NSData *)data publicPemKey:(NSString *)pubKey error:(NSError **)error
{
    ...
    NSData *result = [RSA decryptData:data withKeyRef:publicKey error:error];
...
{
    NSData *encryptData = [RSA decryptWithString:string publicPemKey:pubKey error:error];
    if (encryptData)
    {
...
{
    NSData *encryptData = [RSA decryptWithData:data publicPemKey:pubKey error:error];
    if (encryptData)
    {
        return [[NSString alloc] initWithData:encryptData encoding:NSUTF8StringEncoding];
    }
...
+ (NSData *)decryptWithString:(NSString *)string privatePemKey:(NSString *)privKey error:(NSError **)error
{
    ...
    return [RSA decryptWithData:utfData privatePemKey:privKey error:error];
}
...
+ (NSData *)decryptWithData:(NSData *)data privatePemKey:(NSString *)privKey error:(NSError **)error
{
    ...
    NSData *privateKeyData = [RSA dataTrimFromPrivatePemKey:privKey];
    ...
    NSData *result = [RSA decryptData:data withKeyRef:privateKey error:error];
...
{
    NSData *encryptData = [RSA decryptWithString:string privatePemKey:privKey error:error];
    if (encryptData)
    {
        return [[NSString alloc] initWithData:encryptData encoding:NSUTF8StringEncoding];
    }
...
{
    NSData *encryptData = [RSA decryptWithData:data privatePemKey:privKey error:error];
    if (encryptData)
    {
        return [[NSString alloc] initWithData:encryptData encoding:NSUTF8StringEncoding];
    }
